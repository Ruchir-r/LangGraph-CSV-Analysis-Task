...
Based on specification requirements with clean state management
...
# Global progress callback registry to avoid serialization issues
_progress_callbacks = {}
...
class AnalysisState(TypedDict):
    ...
    All attributes are defined upfront to prevent missing attribute errors.
    """
    # Input parameters
    query: str
    files: List[Dict[str, Any]]  # File metadata with paths
    session_id: str
    execution_id: str
    ...
    # Analysis planning
    operation_type: str  # "single_table", "cross_table", "temporal_comparison"
    analysis_plan: Dict[str, Any]
    target_metrics: List[str]
    
    # Time series alignment
    time_dimension: str  # "month", "quarter", "year"
...
async def send_progress_update(state: AnalysisState, node_name: str, progress_percent: float, message: str):
    ...
    if progress_callback:
        try:
            # Calculate completed nodes for progress
            total_nodes = 8  # parse_files, plan_operations, align_timeseries, generate_code, validate_code, execute_code, trend_analysis, explain_result
            ...
            progress_data = {
                ...
                "progress_percentage": min(overall_progress, 99.0),  # Never show 100% until truly done
    ...
    # Progress callback not available - this is normal for non-WebSocket operations

# Workflow node implementations
async def parse_files_node(state: AnalysisState) -> AnalysisState:
    """
    Node 1: Parse multiple files, extract schemas, align columns
    """
    ...
    try:
        ...
        for file_info in state["files"]:
            ...
            try:
                ...
                # Parse JSON metadata columns if present
                json_columns = []
                parsed_json_fields = {}
                ...
                for col in df.columns:
                    if col.lower() in ['meta', 'metadata', 'json_data']:
                        ...
                        # Try to parse JSON values and extract unique fields
                        json_fields = set()
                        for val in df[col].dropna().head(10):  # Sample first 10 non-null values
                            try:
                ...
                # Extract schema information (without storing DataFrame to avoid serialization issues)
                file_data = {
                    "file_id": file_id,
                    ...
                    # Note: DataFrame is not stored in state to avoid serialization issues
                    # It can be reloaded from file_path when needed
                }
        ...
        # Determine common columns across files
        if parsed_files:
...
async def plan_operations_node(state: AnalysisState) -> AnalysisState:
    """
    Node 2: Plan operations using LLM - decide if single-table or cross-table
    """
...
"""
        ...
        try:
            ...
            # If JSON parsing fails but we got a response, create a basic plan
            ...
            analysis_plan = {
                ...
                "target_metrics": list(set(numeric_cols))[:3],  # Top 3 unique numeric columns
...
async def align_timeseries_node(state: AnalysisState) -> AnalysisState:
    """
    Node 3: Align tables by time dimension before code generation
    """
    logger.info("📅 Starting align_timeseries node")
    
    try:
        if state["operation_type"] == "single_table":
            # No alignment needed for single table
            state.update({
                "current_node": "align_timeseries", 
                "aligned_data": {"single_table": state["parsed_files"][0] if state["parsed_files"] else {}}
            })
        else:
            # For multi-table operations, create aligned data structure
            aligned_data = {}
            
            for file_data in state["parsed_files"]:
                # Extract time period from filename (basic heuristic)
...
async def generate_code_node(state: AnalysisState) -> AnalysisState:
    """
    Node 4a: Generate Python analysis code using LLM with error-aware retry capability
    """
    # Check if this is a retry and increment counter
    retry_count = state.get("generation_retry_count", 0)
    is_retry = retry_count > 0
    
    # Get recent errors for context
    recent_errors = state.get("errors", [])[-5:]  # Last 5 errors
    execution_errors = []
    ...
    try:
        ...
        for time_key, file_data in state["aligned_data"].items():
            files_context.append({
                ...
                "sample_data": file_data.get("data_sample", [])[:2]  # Limited sample
            })
        
        # Get JSON parsing requirements
...
- Group by JSON fields: df.groupby(['channel', 'priority'])
        ...
        # Build error context section if this is a retry
...
"""
        ...
        # Add retry counter to state if not present
        if "code_generation_attempts" not in state:
            state["code_generation_attempts"] = 0
        
        state["code_generation_attempts"] += 1
        
        # If this is a retry due to error, use a much simpler and safer template
        if state["code_generation_attempts"] > 1:
            ...
            # Use a much simpler, safer template for retries
            code_prompt = f"""
...
try:
    ...
    if not dfs:
        ...
        df = dfs[0]  # Use first available DataFrame
        
        # Simple, safe operations only
        if not df.empty and len(df) > 0:
            # Basic stats - SAFE operations only
...
async def validate_code_node(state: AnalysisState) -> AnalysisState:
    """
    Node 4b: Validate generated code for syntax and safety
    """
    ...
    try:
        ...
        # Enhanced safety checks (prevent dangerous operations and infinite loops)
        dangerous_patterns = [
            "import os", "import subprocess", "exec(", "eval(", 
            "open(", "__import__", "getattr(", "delattr(",
            "while True:", "while 1:", "for i in range(999",  # Potential infinite loops
            "time.sleep", "input(", "raw_input(",  # Blocking operations
        ]
...
async def execute_code_node(state: AnalysisState) -> AnalysisState:
    """
    Node 4c: Execute the validated code safely with enhanced error handling and recovery
    """
    # Track execution retry count
    ...
    try:
        # Use enhanced safe execution if available
        if ERROR_HANDLING_AVAILABLE:
            ...
            # Prepare context with dataframes
            context = {'dataframes': {}}
            
            # Load dataframes from file paths into context
            for time_key, file_data in state["aligned_data"].items():
                df_name = f"df_{time_key.replace('-', '_').replace(' ', '_')}"
                try:
                    file_path = Path(file_data['file_path'])
            ...
            if execution_results and execution_results.get('status') in ['success', 'completed', 'completed_with_fallback']:
                ...
                # Add error details to state for debugging
        ...
        # Ensure results are always valid
...
async def trend_analysis_node(state: AnalysisState) -> AnalysisState:
    """
    Node 5: Detect patterns, trends, and anomalies using LLM
    """
    ...
    try:
        ...
        # Analyze execution results for trends - handle JSON serialization
        try:
            results_summary = json.dumps(state["execution_results"], indent=2, default=str)[:1000]  # Limit size
...
"""
        ...
        # Check if we need advanced statistical analysis
        comparison_type = state["analysis_plan"].get("comparison_type", "none")
...
async def explain_result_node(state: AnalysisState) -> AnalysisState:
    """
    Node 6: Generate narrative explanation and recommended actions
    """
    ...
    try:
        ...
        # Compile all results for explanation including statistical analysis
        context = {
            "query": state["query"],
